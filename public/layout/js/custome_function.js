...
$(document).on('click','.add_btn', function(){
  ...
  var arr={};
  if(obj.is_customized=='Yes'){
	  
  }
  else
  {
	
	arr.recipe_id=obj.id;
	arr.recipie_price=parseFloat(obj.price);
	arr.totla_price=parseFloat(obj.price*1);
	arr.qty=1;
	arr.name=obj.name;
	arr.desc=obj.description;
	arr.isCustomized=obj.is_customized;
	arr.cust_core_price=obj.price;
	arr.row_id=parseInt(Date.now() * Math.random());
	arr.options=[];
	arr.isRepeat='false'; 
  }
...
$(document).on('click', '.confirm_order', function()
{
	...
	var vdata = getAllItems()
	if($.isEmptyObject(vdata) && vdata.length==0)
	{
	  alert('Cart is Empty');  
	  return false;
	}
	...
	$.ajax({
	   ...
	   data:{'customer':'','table':tbl,'meta':vdata,'total_amount':get_cart_totalamount()}, // serializes the form's elements.
	   dataType:'json',
	   success: function(data)
	   {
...
$(document).on('click', '.update_qty', function(){
  ...
  if(obj.is_customized=='Yes')
  {
	  arr.recipe_id=obj.id;
	  arr.recipie_price=parseFloat(obj.price);
	  arr.totla_price=parseFloat(obj.price*obj.qty);
	  arr.qty=parseInt(obj.qty);
	  arr.isCustomized=obj.is_customized;
	  arr.cust_core_price=obj.price;
	  arr.isRepeat='false'; 
	  arr.isRepeatelse=(status=='dec')? 'Yes' : 'No';
  ...
  {
	$('#qty'+row_id).val(response.recipe_qty);
  }
...
function addCart(obj)
{
	...
	$.ajax({
	   ...
	   success: function(data)
	   {
		  ...
		  if(data.data!=null)
		  {
			$('.cart-item').text(data.data.total_qty);
			$('.t-price').text(data.data.total_amount);
...
function qtyValidation(obj)
{
  var res=true;	
  var id=$(obj).data('id');
  var pid=$(obj).data('parid');	
  
  var head_obj=$('#head_option_req_'+pid);	
  var qty_sec_obj=$('#qty_rec_'+id);
  
	if(parseInt($('#head_option_req_'+pid).val())== parseInt($('#head_option_req_'+pid).attr('data-qty')))
	{
	  res=false;
	}
  
	//alert(parseInt($('#head_option_req_'+pid).val())+'='+parseInt($('#head_option_req_'+pid).attr('data-qty'))+'='+res)
  return res;
...
function setQty(obj){
	...
	var pid=$(obj).data('parid');
	var head_obj=$('#head_option_req_'+pid);
	var res=qtyValidation(obj)
	
	/*if($('#qty_rec_'+id).length > 0)
	{
	}
	else
	{
		if(res!=false){
		 var hdqty=parseInt(head_obj.data('qty'));
		 head_obj.attr('data-qty',hdqty+1);
		} 
	}*/
	
	qty=($('#qty_rec_'+id).length>0)? $('#qty_rec_'+id).data('qty') :0;
	
	if(res==false && head_obj.data('isrequired')=='Yes'){
	...
	qty=(qty!='')? parseInt(qty)+1 : 1;
	var hdqty=parseInt($('#head_option_req_'+pid).attr('data-qty'))+1;
	
	$('#head_option_req_'+pid).attr('data-qty',hdqty);
	return qty;
}
$(document).on('click','.click_row',function(){
	
	var id=$(this).data('id');
	var pid=$(this).data('parid');
	var qty=setQty(this);
	if(qty==0){
		customTotal()
		return false;
	}
	$('.chk_hidd_'+id).attr('data-ischecked','true')
	$('.chk_hidd_'+id).attr('data-qty',qty)
	$('#del_div_'+id).html('<span class="delete-selection text-danger icofont-trash" id="delete_btn_'+id+'" data-qty="'+qty+'" data-parid="'+pid+'" data-id="'+id+'" style="float:left;"></span>');
	$('#qty_div_'+id).html('<label class="chk_container mt-2 qty_rec" data-qty="'+qty+'" id="qty_rec_'+id+'">X'+qty+'</lable>');
...
$(document).on('click','.delete-selection',function(){
	...
	$('#qty_div_'+id).html('')
	$('.chk_hidd_'+id).attr('data-ischecked','false')
	$('.chk_hidd_'+id).attr('data-qty',0)
	var old_qty=0;
	...
	var hdqty=parseInt($('#head_option_req_'+pid).attr('data-qty'));
	
	
	var res=hdqty-old_qty;
	
	$('#head_option_req_'+pid).attr('data-qty',res);
